...
public class FoldTextView extends AppCompatTextView {
    ...
    public FoldTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            mShowMaxLine = arr.getInt(R.styleable.FoldTextView_show_max_Line, MAX_LINE);
            mTipGravity = arr.getInt(R.styleable.FoldTextView_tip_gravity, END);
            mTipColor = arr.getColor(R.styleable.FoldTextView_tip_color, TIP_COLOR);
            mTipClickable = arr.getBoolean(R.styleable.FoldTextView_tip_clickable, false);
            mFoldText = arr.getString(R.styleable.FoldTextView_fold_text);
            mExpandText = arr.getString(R.styleable.FoldTextView_expand_text);
            isShowTipAfterExpand = arr.getBoolean(R.styleable.FoldTextView_show_tip_after_expand, false);
            arr.recycle();
        }
    ...
    private void translateText(Layout layout, BufferType type) {
        try {
            ...
            if (layout.getLineCount() > mShowMaxLine) {
                ...
                CharSequence ellipsize = mOriginalText.subSequence(0, end-1);
        ...
        if (isOverMaxLine && !isExpand) {
            ...
            if (mTipGravity == END) {
                ...
                canvas.drawText(mFoldText, minX,
                    getHeight() - getPaint().getFontMetrics().descent - getPaddingBottom() - UIUtil.dip2px(getContext(), 2), mPaint);
            } else {
                minX = getPaddingLeft();
                maxX = minX + getTextWidth(mFoldText);
                minY = getHeight() - (getPaint().getFontMetrics().descent - getPaint().getFontMetrics().ascent) - getPaddingBottom();
                maxY = getHeight() - getPaddingBottom();
                canvas.drawText(mFoldText, minX, getHeight() - getPaint().getFontMetrics().descent - getPaddingBottom() - UIUtil.dip2px(getContext(),2), mPaint);
            }
        }
    }
