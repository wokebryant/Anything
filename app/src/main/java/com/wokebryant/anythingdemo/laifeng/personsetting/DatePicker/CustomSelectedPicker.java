package com.wokebryant.anythingdemo.laifeng.personsetting.DatePicker;
 ...
 * 自定义时间选择器
 */
public class CustomSelectedPicker implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    private boolean mIsStringSelected;
     ...
     */
    public CustomSelectedPicker(Context context, StringCallback stringCallback){
        if (context == null || stringCallback == null) {
     ...
     */
    public CustomSelectedPicker(Context context, Callback callback, String beginDateStr, String endDateStr) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tv_confirm:
                if (mIsStringSelected) {
                    if (mStringCallback != null) {
                        mStringCallback.onItemSelected(mSelectedString);
                    }
                } else {
                    if (mCallback != null) {
     ...
     * @param dataList
     */
    public void show(List<String> dataList, int position) {
        if (!canShow() || dataList == null)  {
            return;
        }

        mDpvYear.setDataList(dataList);
        mDpvYear.setSelected(position);
        mPickerDialog.show();
    }
    ...
    /**
     * 设置只显示一个选择器
     * @param showOnePickerView
     */
    public void setOnePickerView(boolean showOnePickerView) {
        if (!canShow()) {
            return;
        }

        mIsStringSelected = showOnePickerView;

        if (showOnePickerView) {
            mDpvHour.setVisibility(View.GONE);
            mTvHourUnit.setVisibility(View.GONE);
            mDpvMinute.setVisibility(View.GONE);
            mTvMinuteUnit.setVisibility(View.GONE);
